...
#### 2.4.1 使用技術スタック
- **VSCode拡張**: TypeScript
- **エージェント実装**: Claude Agent SDK
- **Claude API**: Claude Opus 4.5 / Sonnet 4.5
- **パッケージマネージャー**: npm/pnpm

#### 2.4.2 Claude Agent SDK統合
- 各エージェント（PM、エンジニア、レビュアー、オーケストレーター）を独立したAgentとして実装
- エージェント間通信はオーケストレーターが制御
- コンテキスト共有機構の実装

#### 2.4.3 Claude Code連携
- 既存のClaude Code機能（Read, Write, Edit, Bash等のツール）を各エージェントで利用可能に
- Skillsとの連携（必要に応じてカスタムSkillを作成）
...
### 3.4 セキュリティ
- APIキーの安全な管理（VSCode Secret Storageを利用）
